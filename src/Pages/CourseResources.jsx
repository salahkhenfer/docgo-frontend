...
export default function CourseResources() {
  ...
  const resources = course?.resources || course?.pdfs || course?.documents || course?.files || [];
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">Viewing PDF Document</p>
                </div>
              </div>
              <button
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-slate-200 backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-slate-600 mt-2 text-lg">{t("Download PDFs and course materials")}</p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-16 text-center border border-slate-200">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-slate-600 text-lg">
                {t("There are no PDF resources available for this course yet.")}
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {resources.map((resource, index) => {
              ...
              if (resourceUrl && !resourceUrl.startsWith('http')) {
                resourceUrl = `${import.meta.env.VITE_API_URL}${resourceUrl.startsWith('/') ? '' : '/'}${resourceUrl}`;
              }
              ...
              const handleOpenPDF = () => {
                ...
                setViewingPdf({
                  url: resourceUrl,
                  title: resource.title || resource.name || `Document ${index + 1}`
                });
              };
              ...
              const handleDownloadPDF = () => {
                ...
                link.download = `${resource.title || resource.name || `document-${index + 1}`}.pdf`;
                link.target = '_blank';
