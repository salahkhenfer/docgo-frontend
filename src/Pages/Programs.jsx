...
export function Programs() {
    ...
    const [enrolledProgramsLoading, setEnrolledProgramsLoading] =
        useState(false);
        ...
        async (page = 1, forceRefresh = false, isFilterChange = false) => {
            try {
                ...
                if (hasSearch || hasFilters) {
                    ...
                    response =
                        await clientProgramsAPI.searchPrograms(searchParams);
                } else {
                            ...
                            {enrolledProgramsLoading ? (
                                <div className="flex gap-4 overflow-hidden">
                                    {[1, 2, 3].map((i) => (
                                        <div
                                            key={i}
                                            className="flex-shrink-0 w-80 bg-white/20 rounded-2xl h-28 animate-pulse"
                                        />
                                    ))}
                                </div>
                            ) : (
                                    ...
                                    {enrolledPrograms.map((program) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className="flex-shrink-0 w-80 bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group cursor-pointer"
                                            >
                                                {/* Image strip */}
