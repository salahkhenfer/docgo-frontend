...
export const useProgram = (programId) => {
      ...
      try {
        ...
        if (resp?.data?.data?.applications) {
          ...
          const app = apps.find((a) => {
            if (a.ProgramId && String(a.ProgramId) === String(programId))
              return true;
            if (
              a.Program &&
              a.Program.id &&
            ...
            if (
              ...
              String(a.applicationProgram.id) === String(programId)
            )
              return true;
            return false;
          });
  ...
  const handleFreeProgramApplication = useCallback(async () => {
    try {
      ...
      const response = await axios.post("/enrollment/programs/enroll-free", {
        programId: programId,
      });
      ...
      await Swal.fire({
        title: t("alerts.application.successTitle", "Application Submitted"),
        text: t("alerts.application.successText", "Your application has been submitted successfully"),
        icon: "success",
  ...
  const handleApplyClick = useCallback(async () => {
    ...
    if (!isAuth || !user) {
      const result = await Swal.fire({
        ...
        text: t("alerts.auth.authRequiredText", "Please log in to apply for this program"),
        icon: "info",
    ...
    if (programData?.userStatus?.hasApplied) {
      ...
      if (status === "approved") {
        await Swal.fire({
          title: t("alerts.application.alreadyEnrolledTitle", "Already Enrolled"),
          text: t("alerts.application.alreadyEnrolledText", "You are already enrolled in this program"),
          icon: "info",
          confirmButtonColor: "#3b82f6",
          confirmButtonText: t("alerts.application.viewApplications", "View Applications"),
          customClass: {
